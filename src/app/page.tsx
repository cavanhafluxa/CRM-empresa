...
import {
  ...
  Bell, Search, Plus, ChevronRight, ChevronLeft, TrendingUp,
  DollarSign, Target, MoreHorizontal, X, Edit2, Trash2, Phone,
  Mail, MessageSquare, AlertCircle, ChevronDown, Filter, Download,
  FileText, ArrowUp, ArrowDown, Star, Zap, CheckCircle, RefreshCw,
  Upload, User, Building2, Move, CalendarDays, Briefcase, ChevronUp,
  Sun, Moon, Camera, Shield, UserPlus, Funnel, PenLine, HeadphonesIcon,
  Send, Menu, Eye, EyeOff, Hash, Info
} from 'lucide-react'
...
// ── Constants ─────────────────────────────────────────────────────────────────
const DEFAULT_STAGES = ['Novo Lead','Qualificado','Reunião Marcada','Proposta','Fechado','Perdido']
const STAGE_META: Record<string,{dot:string,bg:string,text:string,border:string}> = {
...
const N8N_WEBHOOK = process.env.NEXT_PUBLIC_N8N_WEBHOOK || 'https://n8n-n8n.uhcnbw.easypanel.host/webhook/d079ece0-6062-4d70-9d05-c850aae89c1b'
...
function Toast({toasts,rm}:any) {
  return (
    <div className="fixed bottom-4 right-4 z-[300] flex flex-col gap-2 max-w-xs w-full">
      {toasts.map((t:any)=>(
        <div key={t.id} className={cls('slide-right flex items-center gap-2 px-4 py-3 rounded-xl shadow-2xl text-sm font-medium border',
          ...
          {t.type==='success'?<CheckCircle size={15}/>:t.type==='error'?<AlertCircle size={15}/>:<Bell size={15}/>}
...
function Sk({className}:{className?:string}) {
  return <div className={cls('animate-pulse rounded-lg bg-white/5',className)}/>
}

function Av({name,url,size='md',color}:{name:string,url?:string,size?:string,color?:string}) {
  ...
  const bg=color||`hsl(${(initials.charCodeAt(0)*43)%360},55%,42%)`
  if(url) return <img src={url} className={cls(s[size],'rounded-xl object-cover shrink-0')} alt={name}/>
  return <div className={cls(s[size],'rounded-xl flex items-center justify-center font-bold shrink-0')} style={{background:bg,color:'#fff'}}>{initials}</div>
}

function Bdg({children,v='default',sm=true}:any) {
  const vs:any={
  ...
  return <span className={cls('inline-flex items-center gap-1 border rounded-full font-medium',sm?'px-2 py-0.5 text-xs':'px-3 py-1 text-sm',vs[v])}>{children}</span>
}

function Modal({open,onClose,title,children,size='md'}:any) {
  if(!open) return null
  const s:any={sm:'max-w-sm',md:'max-w-lg',lg:'max-w-2xl',xl:'max-w-4xl'}
  return (
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={onClose}/>
      <div className={cls('relative w-full bg-slate-900 border border-white/10 shadow-2xl overflow-hidden',
        'rounded-t-2xl sm:rounded-2xl',s[size],'max-h-[92vh] flex flex-col')}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/8 shrink-0">
          <h2 className="text-sm font-semibold text-white">{title}</h2>
          <button onClick={onClose} className="w-7 h-7 rounded-lg hover:bg-white/8 flex items-center justify-center text-slate-400 hover:text-white"><X size={15}/></button>
        </div>
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} size="sm">
      ...
      <div className="flex gap-2 justify-end">
        <button onClick={onClose} className="px-4 py-2 rounded-lg border border-white/10 text-slate-400 text-sm">Cancelar</button>
        <button onClick={onOk} className="px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-medium">Excluir</button>
      </div>
    </Modal>
  )
}
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        <input type={type} value={value??''} onChange={e=>onChange&&onChange(e.target.value)} placeholder={placeholder} disabled={disabled}
          className={cls('w-full rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-slate-600',
            'focus:outline-none focus:border-emerald-500/50 transition-all py-2.5',
  ...
  return (
    <div className="h-14 px-4 flex items-center justify-between border-b border-white/[0.06] bg-slate-950/80 backdrop-blur-xl shrink-0">
      <div className="flex items-center gap-3">
        <button onClick={onMenu} className="sm:hidden w-8 h-8 rounded-lg hover:bg-white/8 flex items-center justify-center text-slate-400"><Menu size={18}/></button>
        <div>
...
function Login({onLogin}:any) {
  ...
  const handle = async () => {
    ...
    try {
      // 1. Find company
      const {data:company,error:ce} = await sb.from('companies').select('*').eq('company_slug',slug.trim().toLowerCase()).eq('crm_active',true).single()
      if(ce||!company){setErr('Empresa não encontrada.');setLoading(false);return}
      // 2. Validate company password OR user credentials
      const {data:auth} = await sb.from('company_auth').select('*').eq('company_slug',slug.trim().toLowerCase()).eq('active',true).single()
      // Try user login first
      const {data:userData} = await sb.from('users').select('*').eq('company_id',company.id).eq('username',user.trim()).eq('password_hash',pass).eq('active',true).single()
      if(userData) {
        await sb.from('company_auth').update({last_login_at:new Date().toISOString()}).eq('company_id',company.id)
        onLogin({company,user:userData})
        setLoading(false); return
      }
      // Try company-level auth (slug+password for founder/gestor)
      if(auth&&auth.password_hash===pass) {
        const {data:firstUser} = await sb.from('users').select('*').eq('company_id',company.id).in('role',['founder','gestor']).eq('active',true).order('created_at').limit(1).single()
        if(firstUser){onLogin({company,user:firstUser});setLoading(false);return}
      }
      setErr('Usuário ou senha inválidos.')
    } catch(e) { setErr('Erro de conexão. Verifique as configurações.') }
    setLoading(false)
  }

  return (
    <div className="min-h-screen bg-[#050812] flex items-center justify-center relative overflow-hidden px-4">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-80 h-80 bg-emerald-500/8 rounded-full blur-3xl"/>
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-blue-500/6 rounded-full blur-3xl"/>
      </div>
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 backdrop-blur-xl">
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input type={showPass?'text':'password'} value={pass} onChange={e=>setPass(e.target.value)}
                  ...
                  className="w-full pl-3 pr-9 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-slate-600 focus:outline-none focus:border-emerald-500/50 transition-all"/>
            ...
            <button onClick={handle} disabled={loading} className="w-full py-2.5 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-sm font-semibold transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-60 mt-1">
          ...
          <p className="text-center text-[10px] text-slate-700 mt-4">Demo: empresa <span className="text-slate-500">fluxa</span> · usuário <span className="text-slate-500">eliezer</span> · senha <span className="text-slate-500">fluxa123</span></p>
        </div>
      </div>
    </div>
  )
...
function Sidebar({active,setActive,company,user,onLogout,open,setOpen,dark,setDark}:any) {
  const canSeeColabs = ['founder','gestor'].includes(user.role)
  const nav = [
    ...
    {id:'suporte',label:'Contatar Suporte',icon:HeadphonesIcon},
  ]
  ...
  const inner = (
    <div className="flex flex-col h-full bg-slate-950 border-r border-white/[0.06]">
      ...
      <div className="flex items-center gap-2.5 px-4 h-14 border-b border-white/[0.06] shrink-0">
        ...
        <button onClick={()=>setOpen(false)} className="sm:hidden ml-auto text-slate-500 hover:text-white"><X size={16}/></button>
      </div>
      {/* Nav */}
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {nav.map(({id,label,icon:Icon})=>(
          <button key={id} onClick={()=>{setActive(id);setOpen(false)}}
            className={cls('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all',
      ...
      <div className="p-2 border-t border-white/[0.06] space-y-1">
        <button onClick={()=>setDark(!dark)} className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-slate-500 hover:text-slate-300 hover:bg-white/[0.04] text-sm">
        ...
        <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/[0.02]">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={cls('text-[10px] font-medium',rc[user.role])}>{rl[user.role]}</p>
          </div>
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({leads,company,addToast,user}:any) {
  ...
  const stageData=DEFAULT_STAGES.map(s=>({name:s,value:leads.filter((l:any)=>l.pipeline_stage===s).length,color:STAGE_META[s]?.dot||'#888'})).filter(d=>d.value>0)
    ...
    <div className="flex-1 overflow-y-auto">
      <Header title="Dashboard" subtitle={company.company_name} actions={
        <div className="flex gap-1.5">
          <button onClick={exportCSV} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-white/10 text-slate-400 hover:text-white text-xs transition-all"><Download size={12}/>CSV</button>
          <button onClick={exportPDF} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs transition-all"><FileText size={12}/>Report</button>
        </div>
      }/>
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            {label:'Total Leads',val:loading?'—':total,change:12,icon:Users,color:'from-blue-600/20 to-blue-900/10'},
            {label:'Conversão',val:loading?'—':`${conv}%`,change:3,icon:Target,color:'from-emerald-600/20 to-emerald-900/10'},
            {label:'Ticket Médio',val:loading?'—':`R$${ticket.toLocaleString()}`,change:8,icon:DollarSign,color:'from-violet-600/20 to-violet-900/10'},
            {label:'Negociação',val:loading?'—':`R$${(negoc/1000).toFixed(0)}k`,change:-2,icon:TrendingUp,color:'from-amber-600/20 to-amber-900/10'},
          ].map(({label,val,change,icon:Icon,color})=>(
            <div key={label} className={cls('p-4 rounded-2xl border border-white/[0.07] bg-gradient-to-br relative overflow-hidden',color)}>
              <div className="flex items-center justify-between mb-2">
                <div className="w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center"><Icon size={14} className="text-white"/></div>
                <span className={cls('flex items-center gap-0.5 text-[10px] font-medium',change>=0?'text-emerald-400':'text-red-400')}>
        ...
        <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02]">
          <p className="text-xs font-semibold text-white mb-3">Evolução de Leads</p>
          {loading?<Sk className="h-36"/>:(
            <ResponsiveContainer width="100%" height={140}>
              <AreaChart data={CHART_DATA}>
                <defs>
                  <linearGradient id="g1" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#34D399" stopOpacity={0.3}/><stop offset="95%" stopColor="#34D399" stopOpacity={0}/></linearGradient>
                </defs>
          ...
          <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02]">
            ...
            <div className="flex items-center gap-4">
              <ResponsiveContainer width={120} height={120}>
                <PieChart>
                  <Pie data={stageData} cx="50%" cy="50%" innerRadius={35} outerRadius={55} paddingAngle={3} dataKey="value">
              ...
              <div className="flex-1 space-y-1.5">
                {stageData.map((s:any,i:number)=>(
                  <div key={i} className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full shrink-0" style={{background:s.color}}/><span className="text-slate-400 truncate text-[11px]">{s.name}</span></div>
        ...
        <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02]">
          ...
          <div className="space-y-2">
            {leads.slice(0,5).map((l:any)=>{
              const m=STAGE_META[l.pipeline_stage]||STAGE_META['Novo Lead']
              return (
                <div key={l.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/[0.03]">
                  <Av name={l.nome} size="sm"/>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-white truncate">{l.nome}</p>
                    <p className="text-[10px] text-slate-500 truncate">{l.servico||'—'}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs font-bold text-white">R${(l.valor_estimado||0).toLocaleString()}</p>
                    <span className={cls('text-[9px] font-medium px-1.5 py-0.5 rounded-full border',m.bg,m.text,m.border)}>{l.pipeline_stage}</span>
                  </div>
                </div>
              )
            })}
...
// ── LEAD MODAL ────────────────────────────────────────────────────────────────
function LeadModal({lead,open,onClose,onSave,onDelete,role,addToast,funnels,users,companyId}:any) {
  ...
  useEffect(()=>{
    if(lead){setForm(lead);loadNotes(lead.id)}
  },[lead])
  ...
  const save = async()=>{
    ...
    const {error}=await sb.from('leads').update({
      ...
      servico:form.servico,valor_estimado:form.valor_estimado,
      ...
      funnel_id:form.funnel_id,assigned_to:form.assigned_to,
  ...
  const addNote = async()=>{
    ...
    const {data,error}=await sb.from('lead_notes').insert({
      company_id:companyId,lead_id:lead.id,note:note.trim(),user_name:'Você'
    }).select().single()
    if(!error&&data){setNotes(n=>[data,...n]);setNote('');addToast('Anotação adicionada!','success')}
  }
  ...
  return (
    <>
      <Modal open={open} onClose={onClose} title={lead.nome} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Valor" value={form.valor_estimado} onChange={(v:string)=>set('valor_estimado',Number(v))} type="number" icon={DollarSign}/>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Sel label="Etapa" value={form.pipeline_stage} onChange={(v:string)=>set('pipeline_stage',v)} options={DEFAULT_STAGES.map(s=>({value:s,label:s}))}/>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-slate-400">Interesse</label>
            <div className="flex gap-1">{[1,2,3,4,5].map(n=><button key={n} onClick={()=>set('nivel_interesse',n)}><Star size={20} fill={n<=(form.nivel_interesse||0)?'#FBBF24':'none'} stroke={n<=(form.nivel_interesse||0)?'#FBBF24':'#475569'}/></button>)}</div>
          </div>
          <Textarea label="Observações" value={form.resumo_gestor} onChange={(v:string)=>set('resumo_gestor',v)} placeholder="Resumo do gestor..."/>
          ...
          <div className="border-t border-white/[0.06] pt-4">
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto mb-3">
              {notes.map((n:any)=>(
                <div key={n.id} className="p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                  <div className="flex justify-between mb-1"><span className="text-[10px] font-medium text-slate-300">{n.user_name||'Sistema'}</span><span className="text-[10px] text-slate-600">{n.created_at?.split('T')[0]}</span></div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={addNote} className="px-3 py-2 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs hover:bg-emerald-500/25"><Send size={13}/></button>
            </div>
          </div>
        </div>
      ...
      <Confirm open={delConfirm} onClose={()=>setDelConfirm(false)} onOk={()=>{onDelete(lead.id);setDelConfirm(false);onClose();addToast('Lead excluído.','error')}} title="Excluir Lead" msg={`Excluir "${lead.nome}"? Não pode ser desfeito.`}/>
    </>
  )
}
...
function Pipeline({leads,setLeads,role,addToast,company,funnels,users}:any) {
  ...
  // Load column labels from supabase
  useEffect(()=>{
  ...
  const getLabel=(stage:string)=>colLabels[stage]||stage

  const saveColLabel = async(stage:string,label:string)=>{
    if(!canEdit) return
    await sb.from('pipeline_columns').upsert({company_id:company.id,stage_key:stage,label:label},{onConflict:'company_id,stage_key'})
    setColLabels(prev=>({...prev,[stage]:label}))
    setEditCol(null)
    addToast('Coluna renomeada!','success')
  }
  ...
  const drop = async(stage:string)=>{
    ...
    if(!error){setLeads((ls:any)=>ls.map((l:any)=>l.id===dragId?{...l,pipeline_stage:stage}:l));addToast(`Movido para ${getLabel(stage)}!`,'success')}
    setDragId(null);setDragOver(null)
  }

  const create = async()=>{
    if(!newLead.nome?.trim()) return addToast('Nome é obrigatório.','error')
    setCreating(true)
    const {data,error}=await sb.from('leads').insert({
      company_id:company.id,...newLead,
      pipeline_stage:newLead.pipeline_stage||'Novo Lead',
      status:'ativo',nivel_interesse:3,
    }).select().single()
    setCreating(false)
    if(error){addToast('Erro ao criar lead.','error');return}
    setLeads((ls:any)=>[data,...ls])
    setNewLead({});setShowNew(false)
    addToast('Lead criado!','success')
  }
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-x-auto p-3">
        <div className="flex gap-2.5 h-full" style={{minWidth:`${DEFAULT_STAGES.length*200}px`}}>
          {DEFAULT_STAGES.map(stage=>{
            ...
            return (
              <div key={stage} className={cls('flex flex-col rounded-2xl border transition-all flex-1 min-w-[175px]',isOver?'border-white/20 bg-white/[0.05]':'border-white/[0.06] bg-white/[0.015]')}
                    ...
                    <div className="flex items-center gap-1.5 flex-1 min-w-0">
                      ...
                      {editCol===stage&&canEdit?(
                        <input autoFocus value={editVal} onChange={e=>setEditVal(e.target.value)}
                          onBlur={()=>saveColLabel(stage,editVal||stage)}
                          onKeyDown={e=>{if(e.key==='Enter')saveColLabel(stage,editVal||stage);if(e.key==='Escape')setEditCol(null)}}
                          className="flex-1 bg-transparent text-white text-xs font-semibold focus:outline-none border-b border-emerald-500/50"/>
                      ):(
                        <span className="text-xs font-semibold text-white truncate">{getLabel(stage)}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      {canEdit&&<button onClick={()=>{setEditCol(stage);setEditVal(getLabel(stage))}} className="text-slate-600 hover:text-slate-300 transition-colors"><PenLine size={11}/></button>}
                      <span className={cls('text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shrink-0',m.bg,m.text)}>{sl.length}</span>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                  {sl.map((lead:any)=>(
                    <div key={lead.id} draggable onDragStart={()=>setDragId(lead.id)} onClick={()=>setSelected(lead)}
                      className="p-3 rounded-xl bg-slate-900/80 border border-white/[0.07] hover:border-white/[0.15] cursor-pointer transition-all active:scale-95 active:opacity-70">
                      ...
                      {lead.origem&&<p className="text-[9px] text-slate-600 mt-1.5 pt-1.5 border-t border-white/[0.04]">{lead.origem}</p>}
                    </div>
                  ))}
                  {sl.length===0&&(
                    <div className={cls('flex flex-col items-center justify-center py-6 rounded-xl border-2 border-dashed transition-all',isOver?'border-white/20 bg-white/[0.03]':'border-white/[0.04]')}>
                      <Move size={14} className="text-slate-700 mb-1"/>
                      <p className="text-[10px] text-slate-700">Arraste aqui</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      <Modal open={showNew} onClose={()=>setShowNew(false)} title="Novo Lead" size="md">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Valor" value={newLead.valor_estimado} onChange={(v:string)=>setNewLead((f:any)=>({...f,valor_estimado:Number(v)}))} type="number" icon={DollarSign}/>
            <Sel label="Etapa" value={newLead.pipeline_stage||'Novo Lead'} onChange={(v:string)=>setNewLead((f:any)=>({...f,pipeline_stage:v}))} options={DEFAULT_STAGES.map(s=>({value:s,label:s}))}/>
          </div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex gap-2 justify-end pt-2">
            <button onClick={()=>setShowNew(false)} className="px-4 py-2 rounded-lg border border-white/10 text-slate-400 text-sm">Cancelar</button>
            <button onClick={create} disabled={creating} className="px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium disabled:opacity-60">{creating?'Criando...':'Criar'}</button>
          </div>
        </div>
      </Modal>
      ...
      <LeadModal lead={selected} open={!!selected} onClose={()=>setSelected(null)}
        ...
        onDelete={id=>{setLeads((ls:any)=>ls.filter((l:any)=>l.id!==id));addToast('Excluído.','error')}}
        role={role} addToast={addToast} funnels={funnels} users={users} companyId={company.id}/>
    </div>
  )
}

// ── LEADS TABLE ───────────────────────────────────────────────────────────────
function LeadsTable({leads,setLeads,role,addToast,company,funnels,users}:any) {
  ...
  const filtered=leads
    ...
    .sort((a:any,b:any)=>{const va=a[sortBy],vb=b[sortBy];return sortDir==='asc'?(va>vb?1:-1):(va<vb?1:-1)})
  ...
  const exportCSV=()=>{
    const r=[['Nome','Tel','Serviço','Valor','Etapa','Status','Criado'],...filtered.map((l:any)=>[l.nome,l.telefone||'',l.servico||'',l.valor_estimado||0,l.pipeline_stage,l.status,l.created_at?.split('T')[0]||''])]
    const a=document.createElement('a');a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(r.map(x=>x.join(',')).join('\n'));a.download='leads.csv';a.click()
    addToast('CSV exportado!','success')
  }
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      <Header title="Leads" subtitle={`${filtered.length} registros`} actions={
        <button onClick={exportCSV} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-white/10 text-slate-400 text-xs"><Download size={12}/>CSV</button>
      }/>
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-4">
          ...
          <select value={filterStage} onChange={e=>setFilterStage(e.target.value)}
            className="px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-sm text-slate-300 focus:outline-none appearance-none">
            <option value="all" className="bg-slate-900">Todas etapas</option>
            {DEFAULT_STAGES.map(s=><option key={s} value={s} className="bg-slate-900">{s}</option>)}
          </select>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.07] bg-white/[0.02]">
                {[{k:'nome',l:'Nome'},{k:'servico',l:'Serviço'},{k:'pipeline_stage',l:'Etapa'},{k:'valor_estimado',l:'Valor'},{k:'status',l:'Status'},{k:'created_at',l:'Criado'}].map(c=>(
                  <th key={c.k} onClick={()=>ts(c.k)} className="text-left px-4 py-3 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-300 whitespace-nowrap">
            ...
            <tbody>
              {paged.map((l:any,i:number)=>{
                const m=STAGE_META[l.pipeline_stage]||STAGE_META['Novo Lead']
                return (
                  <tr key={l.id} className={cls('border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors',i%2===0?'':'bg-white/[0.01]')}>
                    ...
                    <td className="px-4 py-3"><span className={cls('text-[10px] font-medium px-2 py-0.5 rounded-full border',m.bg,m.text,m.border)}>{l.pipeline_stage}</span></td>
                    ...
                    <td className="px-4 py-3"><button onClick={()=>setSelected(l)} className="w-7 h-7 rounded-lg hover:bg-white/8 flex items-center justify-center text-slate-500 hover:text-white"><Edit2 size={13}/></button></td>
                  </tr>
                )
              })}
            </tbody>
          </table>
          {paged.length===0&&<div className="py-12 text-center"><Users size={24} className="text-slate-700 mx-auto mb-2"/><p className="text-sm text-slate-600">Nenhum lead</p></div>}
        </div>
            ...
            return (
              <div key={l.id} onClick={()=>setSelected(l)} className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02] active:bg-white/[0.04]">
                ...
                <div className="flex items-center gap-2">
                  <span className={cls('text-[10px] font-medium px-2 py-0.5 rounded-full border',m.bg,m.text,m.border)}>{l.pipeline_stage}</span>
        ...
        {pages>1&&(
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({length:pages},(_,i)=><button key={i} onClick={()=>setPage(i+1)} className={cls('w-7 h-7 rounded-lg text-xs font-medium',page===i+1?'bg-emerald-500 text-white':'text-slate-500 hover:bg-white/8')}>{i+1}</button>)}
...
function CalendarView({company,addToast,user}:any) {
  ...
  const cells:any[]=[...Array(fd).fill(null),...Array.from({length:dim},(_,i)=>i+1)]
  ...
  const create=async()=>{
    if(!form.title?.trim()) return addToast('Título obrigatório.','error')
    ...
    setMeetings(ms=>[...ms,data]);setForm({});setShowNew(false);addToast('Evento criado!','success')
  }
    ...
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-white/[0.01] overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-white/[0.06]">
            <button onClick={()=>setDate(new Date(y,m-1))} className="w-8 h-8 rounded-lg hover:bg-white/8 flex items-center justify-center text-slate-400"><ChevronLeft size={16}/></button>
            <p className="text-sm font-semibold text-white">{months[m]} {y}</p>
            <button onClick={()=>setDate(new Date(y,m+1))} className="w-8 h-8 rounded-lg hover:bg-white/8 flex items-center justify-center text-slate-400"><ChevronRight size={16}/></button>
          </div>
          ...
          {weeks.map((week,wi)=>(
            <div key={wi} className="grid grid-cols-7 border-b border-white/[0.04] last:border-0">
              {week.map((day:any,di:number)=>{
                ...
                return (
                  <div key={di} className={cls('min-h-[56px] p-1 border-r border-white/[0.04] last:border-0',day?'hover:bg-white/[0.02]':'opacity-0')}>
                    {day&&<>
                      <span className={cls('text-[11px] font-medium w-5 h-5 flex items-center justify-center rounded-full mb-0.5',isToday?'bg-emerald-500 text-white':'text-slate-500')}>{day}</span>
                      {dm.slice(0,2).map((mt:any)=><button key={mt.id} onClick={()=>setSel(mt)} className={cls('w-full text-left text-[8px] font-medium px-1 py-0.5 rounded border truncate mb-0.5',tc[mt.event_type]||'bg-white/5 text-white border-white/10')}>{mt.title}</button>)}
        ...
        <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.01]">
          ...
          <div className="space-y-2">
            {meetings.sort((a,b)=>new Date(a.start_at).getTime()-new Date(b.start_at).getTime()).slice(0,5).map(mt=>(
              <button key={mt.id} onClick={()=>setSel(mt)} className="w-full text-left p-3 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:border-white/[0.1] transition-all">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className={cls('text-[10px] font-medium',sc[mt.status])}>{mt.status}</span>
                  </div>
                </div>
              </button>
            ))}
      ...
      <Modal open={showNew} onClose={()=>setShowNew(false)} title="Novo Evento" size="md">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Sel label="Tipo" value={form.event_type||'reuniao'} onChange={(v:string)=>setForm((f:any)=>({...f,event_type:v}))} options={[{value:'reuniao',label:'Reunião'},{value:'visita',label:'Visita'},{value:'instalacao',label:'Instalação'}]}/>
            <Sel label="Status" value={form.status||'pendente'} onChange={(v:string)=>setForm((f:any)=>({...f,status:v}))} options={[{value:'pendente',label:'Pendente'},{value:'confirmado',label:'Confirmado'},{value:'cancelado',label:'Cancelado'}]}/>
          </div>
          <div className="grid grid-cols-2 gap-3">
      ...
      <Modal open={!!sel} onClose={()=>setSel(null)} title="Evento" size="sm">
        {sel&&<div className="space-y-3">
          <div className={cls('p-2.5 rounded-xl border text-xs font-medium',tc[sel.event_type])}>{tl[sel.event_type]}</div>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between text-xs"><span className="text-slate-500">Início</span><span className="text-white">{sel.start_at?.replace('T',' ')}</span></div>
            <div className="flex justify-between text-xs"><span className="text-slate-500">Status</span><span className={cls('font-medium',sc[sel.status])}>{sel.status}</span></div>
            {sel.description&&<p className="text-xs text-slate-400 pt-2 border-t border-white/[0.06]">{sel.description}</p>}
          </div>
          <div className="flex gap-2">
            <button onClick={()=>updateStatus(sel.id,'cancelado')} className="flex-1 py-2 rounded-lg border border-red-500/30 text-red-400 text-sm">Cancelar</button>
            <button onClick={()=>updateStatus(sel.id,'confirmado')} className="flex-1 py-2 rounded-lg bg-emerald-500 text-white text-sm">Confirmar</button>
          </div>
        </div>}
      </Modal>
    </div>
...
// ── COLABORADORES ─────────────────────────────────────────────────────────────
function Colaboradores({company,user,addToast,funnels}:any) {
  ...
  const create=async()=>{
    if(!form.full_name?.trim()||!form.username?.trim()||!form.password_hash?.trim()) return addToast('Nome, usuário e senha obrigatórios.','error')
    setCreating(true)
    const {data,error}=await sb.from('users').insert({
      company_id:company.id,
      full_name:form.full_name,display_name:form.display_name||form.full_name,
      email:form.email||'',username:form.username,
      password_hash:form.password_hash,role:form.role||'colaborador',active:true
    }).select().single()
    setCreating(false)
    if(error){addToast('Erro ao cadastrar.','error');return}
    setColabs(c=>[...c,data]);setForm({});setShowNew(false);addToast('Colaborador cadastrado!','success')
  }

  const updateRole=async(uid:string,role:string)=>{
    const {error}=await sb.from('users').update({role,updated_at:new Date().toISOString()}).eq('id',uid)
    if(!error){setColabs(cs=>cs.map(c=>c.id===uid?{...c,role}:c));setEditRole(null);addToast('Cargo atualizado!','success')}
    else addToast('Erro ao atualizar.','error')
  }

  const deactivate=async(uid:string)=>{
    const {error}=await sb.from('users').update({active:false}).eq('id',uid)
    if(!error){setColabs(cs=>cs.filter(c=>c.id!==uid));addToast('Colaborador removido.','error')}
  }

  const rl:any={founder:'Founder',gestor:'Gestor',colaborador:'Colaborador'}
  const rc:any={founder:'violet',gestor:'info',colaborador:'default'}

  const countLeads=(uid:string)=>0 // would query leads table
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading?<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{[1,2,3].map(i=><Sk key={i} className="h-24"/>)}</div>:(
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {colabs.map(c=>(
              <div key={c.id} className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02] fade-in">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {c.display_name&&c.display_name!==c.full_name&&<p className="text-xs text-slate-500">{c.display_name}</p>}
                    <p className="text-[10px] text-slate-600 font-mono">@{c.username||'—'}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <Bdg v={rc[c.role]}>{rl[c.role]}</Bdg>
                    </div>
                  </div>
                  {canManage&&c.id!==user.id&&(
                    <div className="flex flex-col gap-1">
                      <button onClick={()=>setEditRole(c)} className="w-7 h-7 rounded-lg hover:bg-white/8 flex items-center justify-center text-slate-500 hover:text-violet-400" title="Alterar cargo"><Shield size={12}/></button>
                      {user.role==='founder'&&<button onClick={()=>deactivate(c.id)} className="w-7 h-7 rounded-lg hover:bg-red-500/10 flex items-center justify-center text-slate-500 hover:text-red-400" title="Remover"><Trash2 size={12}/></button>}
                    </div>
                  )}
                </div>
                {c.email&&<p className="text-[10px] text-slate-600 mt-2 flex items-center gap-1"><Mail size={10}/>{c.email}</p>}
              </div>
            ))}
          </div>
        )}
      ...
      <Modal open={showNew} onClose={()=>setShowNew(false)} title="Cadastrar Colaborador" size="md">
        <div className="space-y-3">
          <Field label="Nome completo" value={form.full_name} onChange={(v:string)=>setForm((f:any)=>({...f,full_name:v}))} required icon={User}/>
          <Field label="Nome de exibição" value={form.display_name} onChange={(v:string)=>setForm((f:any)=>({...f,display_name:v}))} placeholder="Apelido ou nome curto"/>
          <Field label="E-mail" value={form.email} onChange={(v:string)=>setForm((f:any)=>({...f,email:v}))} icon={Mail} type="email"/>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Usuário" value={form.username} onChange={(v:string)=>setForm((f:any)=>({...f,username:v}))} required icon={Hash}/>
            <Field label="Senha" value={form.password_hash} onChange={(v:string)=>setForm((f:any)=>({...f,password_hash:v}))} required type="password"/>
          </div>
          <Sel label="Cargo" value={form.role||'colaborador'} onChange={(v:string)=>setForm((f:any)=>({...f,role:v}))} options={[{value:'colaborador',label:'Colaborador'},{value:'gestor',label:'Gestor'},{value:'founder',label:'Founder'}]}/>
          <div className="flex gap-2 justify-end pt-2">
            <button onClick={()=>setShowNew(false)} className="px-4 py-2 rounded-lg border border-white/10 text-slate-400 text-sm">Cancelar</button>
            <button onClick={create} disabled={creating} className="px-4 py-2 rounded-lg bg-emerald-500 text-white text-sm font-medium disabled:opacity-60">{creating?'Cadastrando...':'Cadastrar'}</button>
          </div>
        </div>
      </Modal>

      {/* Edit role modal */}
      <Modal open={!!editRole} onClose={()=>setEditRole(null)} title={`Cargo de ${editRole?.full_name}`} size="sm">
        {editRole&&(
          <div className="space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={()=>setEditRole(null)} className="px-4 py-2 rounded-lg border border-white/10 text-slate-400 text-sm">Cancelar</button>
              <button onClick={()=>updateRole(editRole.id,editRole.role)} className="px-4 py-2 rounded-lg bg-emerald-500 text-white text-sm font-medium">Salvar</button>
            </div>
          </div>
        )}
      </Modal>
...
function SettingsPage({user,company,dark,setDark,onLogout,addToast,setUser,setCompany}:any) {
  ...
  const saveUser=async()=>{
    setSaving(true)
    const {error}=await sb.from('users').update({display_name:uForm.display_name,email:uForm.email,updated_at:new Date().toISOString()}).eq('id',user.id)
    setSaving(false)
    if(error){addToast('Erro ao salvar.','error');return}
    setUser((u:any)=>({...u,...uForm}))
    addToast('Perfil salvo!','success')
  }

  const saveCompany=async()=>{
    setSavingC(true)
    const {error}=await sb.from('companies').update({company_name:cForm.company_name,updated_at:new Date().toISOString()}).eq('id',company.id)
    setSavingC(false)
    if(error){addToast('Erro ao salvar.','error');return}
    setCompany((c:any)=>({...c,...cForm}))
  ...
  const handleAvatar=async(e:any)=>{
    ...
    try {
      const url=await uploadFile(file,'avatars',`${user.id}/avatar.${file.name.split('.').pop()}`)
      await sb.from('users').update({avatar_url:url}).eq('id',user.id)
      setUser((u:any)=>({...u,avatar_url:url}))
      addToast('Foto atualizada!','success')
    } catch { addToast('Erro no upload. Verifique o bucket "avatars" no Supabase.','error') }
  ...
  const handleLogo=async(e:any)=>{
    ...
    try {
      const url=await uploadFile(file,'logos',`${company.id}/logo.${file.name.split('.').pop()}`)
    ...
    } catch { addToast('Erro no upload. Verifique o bucket "logos" no Supabase.','error') }
      ...
      <div className="p-4 space-y-4 max-w-lg">
        ...
        <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.01]">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="relative">
              ...
              <label className={cls('absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-emerald-500 flex items-center justify-center cursor-pointer shadow-md',uploadingAvatar&&'opacity-50')}>
                {uploadingAvatar?<RefreshCw size={10} className="text-white animate-spin"/>:<Camera size={10} className="text-white"/>}
                <input type="file" accept="image/*" className="hidden" onChange={handleAvatar} disabled={uploadingAvatar}/>
              </label>
            </div>
            <div>
              <p className="text-sm font-semibold text-white">{user.full_name}</p>
              <span className={cls('text-xs font-medium px-2 py-0.5 rounded-full border mt-1 inline-block',rc[user.role])}>{rl[user.role]}</span>
            </div>
          </div>
          <div className="space-y-3">
            <Field label="Nome de exibição" value={uForm.display_name} onChange={(v:string)=>setUForm(f=>({...f,display_name:v}))} icon={User}/>
            <Field label="E-mail" value={uForm.email} onChange={(v:string)=>setUForm(f=>({...f,email:v}))} icon={Mail} type="email"/>
            <Field label="Cargo" value={rl[user.role]} onChange={()=>{}} disabled/>
          </div>
          <button onClick={saveUser} disabled={saving} className="mt-4 px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium disabled:opacity-60">{saving?'Salvando...':'Salvar perfil'}</button>
        </div>
          ...
          <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.01]">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="relative">
                ...
                <label className={cls('absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-emerald-500 flex items-center justify-center cursor-pointer shadow-md',uploadingLogo&&'opacity-50')}>
                  {uploadingLogo?<RefreshCw size={10} className="text-white animate-spin"/>:<Upload size={10} className="text-white"/>}
            ...
            <button onClick={saveCompany} disabled={savingC} className="mt-3 px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium disabled:opacity-60">{savingC?'Salvando...':'Salvar empresa'}</button>
          </div>
        )}
        ...
        <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.01]">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={()=>setDark(!dark)} className={cls('relative w-11 h-6 rounded-full transition-all',dark?'bg-emerald-500':'bg-slate-700')}>
              <div className={cls('absolute top-1 w-4 h-4 rounded-full bg-white shadow-md transition-all',dark?'left-6':'left-1')}/>
            </button>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl border border-red-500/10 bg-red-500/[0.02]">
          <button onClick={onLogout} className="flex items-center gap-2 text-red-400 hover:bg-red-500/10 px-3 py-2 rounded-lg text-sm transition-all"><LogOut size={14}/>Sair da conta</button>
        </div>

        {/* Storage note */}
        <div className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/15 flex gap-2.5">
          <Info size={14} className="text-blue-400 shrink-0 mt-0.5"/>
          <p className="text-[11px] text-slate-400">Para upload de fotos funcionar, crie os buckets <span className="text-white font-mono">avatars</span> e <span className="text-white font-mono">logos</span> no Supabase Storage com acesso público.</p>
        </div>
      </div>
    </div>
  )
...
function Suporte({company,user,addToast}:any) {
  ...
  const send=async()=>{
    if(!msg.trim()) return addToast('Digite uma mensagem.','error')
    setSending(true)
    // Save to supabase
    ...
    } catch(e) { /* webhook error is non-blocking */ }
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      <Header title="Suporte" subtitle="Fale com a Flüxa"/>
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {/* Send message */}
        <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.01]">
          <p className="text-xs font-semibold text-white mb-3 flex items-center gap-2"><HeadphonesIcon size={13}/>Nova mensagem</p>
          <Textarea value={msg} onChange={setMsg} placeholder="Descreva sua dúvida ou problema..." rows={4}/>
          <button onClick={send} disabled={sending} className="mt-3 flex items-center gap-2 px-4 py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium disabled:opacity-60 w-full justify-center">
        ...
        <div className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/15 flex gap-2.5">
          <Info size={14} className="text-blue-400 shrink-0 mt-0.5"/>
          <p className="text-xs text-slate-400">Sua mensagem será recebida pela equipe Flüxa via WhatsApp. Respondemos em até 24h úteis.</p>
        </div>
...
export default function App() {
  ...
  const loadData=async(companyId:string)=>{
    setLoading(true)
    const [leadsR,funnelsR,usersR]=await Promise.all([
    ...
    if(funnelsR.data) setFunnels(funnelsR.data)
    if(usersR.data) setUsers(usersR.data)
    setLoading(false)
  }

  const login=({company,user}:any)=>{
    setSession({company,user});loadData(company.id);addToast(`Bem-vindo, ${user.display_name||user.full_name}! 👋`,'success')
  }
  ...
  const pages:any={
    dashboard:<Dashboard leads={leads} company={company} addToast={addToast} user={user}/>,
    pipeline:<Pipeline leads={leads} setLeads={setLeads} role={user.role} addToast={addToast} company={company} funnels={funnels} users={users}/>,
    leads:<LeadsTable leads={leads} setLeads={setLeads} role={user.role} addToast={addToast} company={company} funnels={funnels} users={users}/>,
    calendar:<CalendarView company={company} addToast={addToast} user={user}/>,
    colaboradores:<Colaboradores company={company} user={user} addToast={addToast} funnels={funnels}/>,
    ...
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif"}}>
      <div className="flex h-screen w-screen overflow-hidden bg-[#070C18]">
        <div className="fixed inset-0 pointer-events-none z-0">
          <div className="absolute top-0 left-1/3 w-96 h-64 bg-emerald-500/[0.04] rounded-full blur-3xl"/>
          <div className="absolute bottom-0 right-1/4 w-80 h-80 bg-blue-500/[0.03] rounded-full blur-3xl"/>
        </div>
